...
TEST(HigherTypeSystem, TemplateTypes2) {
  GeneralizedTypeSystem type_system;
  auto pair_tc   = type_system.GetObjectType("pair", "std", 2);
  auto vector_tc = type_system.GetObjectType("vector", "std", 1);
  auto int_t     = type_system.GetBasicType("int");
...
TEST(HigherTypeSystem, Enumeration) {
  ...
  ASSERT_EQ(options[0].first, "RED");
  ASSERT_EQ(options[0].second->Write(), "int");
  ASSERT_EQ(options[1].first, "GREEN");
  ASSERT_EQ(options[1].second->Write(), "int");
  ASSERT_EQ(options[2].first, "BLUE");
  ASSERT_EQ(options[2].second->Write(), "int");
}
...
TEST(HigherTypeSystem, Object) {
  ...
  // Add a constructor.
  EXPECT_NO_THROW(my_class_proto->AddConstructor(StructureConstructor {}.WithArguments(
      {{ElaboratedType(string_t), "field"}, {ElaboratedType(my_class), "next"}})));
