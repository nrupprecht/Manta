...
void ParserCodegen::GenerateParserCode(std::ostream& code_out, const std::shared_ptr<const ParserData>& parser_data) const {
  ...
  code_out << "// ========================================================================\n";
  code_out << std::endl;
  code_out << "class Parser : public manta::ParserDriverBase<ASTNodeBase, Parser> {\n";
  code_out << "public:\n";
  ...
  code_out << "  std::shared_ptr<ASTNodeBase> reduce(unsigned reduction_id, const std::vector<std::shared_ptr<ASTNodeBase>>& collected_nodes);\n\n";
  ...
  code_out << "std::shared_ptr<ASTNodeBase> Parser::reduce(unsigned reduction_id, const std::vector<std::shared_ptr<ASTNodeBase>>& collected_nodes) {\n";
      ...
      switch (rel.check_type) {
        ...
        case CheckType::Append: {
          ...
          code_out << "  new_node->" << field_name << ".insert("
                   ...
                   << arg_name << "->" << *rel.source_field_name << ".cbegin(), "
                   << arg_name << "->" << *rel.source_field_name << ".cend());\n";
          break;
        }
