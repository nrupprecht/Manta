...
ASTNodeManager::ASTNodeManager() {
  ...
  node_base_->AddConstructor(
      StructureConstructor {{{node_enum_, "node_type"}},  // Arguments
                            {},  // Parent constructors
                            {
                                {"node_type", "node_type"},
                            },
...
TypeDescriptionStructure* ASTNodeManager::GetNodeDescription(
    const std::string& type_name, NonterminalID nonterminal_id) {
...
NO_DISCARD TypeDescriptionStructure* ASTNodeManager::GetNodeDescription(
    const std::string& type_name) {
  return type_system_.Structure(type_name);
}

void ASTNodeManager::CreateAllDefinitions(std::ostream& out,
                                          const CodeGen& code_gen) const {
  ...
  for (auto& [nonterminal_id, nonterminal_types] : node_types_for_nonterminal_) {
    ...
    if (nonterminal_types.base_type) {
      LOG_SEV(Info)
          << "There was a parent class for nodes associated with nontermainl type "
