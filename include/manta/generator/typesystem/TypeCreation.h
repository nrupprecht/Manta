...
class ParserDataToTypeManager {
 public:
  ...
  //! \brief Based on the parser description, creates a collection of data that is needed for managing
  //! the parser AST nodes and determining their types.
  //!
  //! Returns (in a struct)
  //! node_manager: An ASTNodeManager that
  //! relationships: All the relationships between fields in different nodes, tells us how to construct one node from another.
  //! nonterminals_for_type: the nonterminal ID associated with each type. In general, since every non-terminal can have
  //!     multiple productions, and each production can result in a different node type, there can be multiple types
  //!     with the same nonterminal ID (each from a different production for the same type).
  ...
  //! \brief Function that deduces what types all fields of all AST nodes must have, or raises that the type constraints cannot be
  //! satisfied.
  //!
 ...
 private:
  ...
  std::function<std::string(const std::string&)> field_name_sanitizer{};
  ...
  std::map<std::string, std::vector<TypeRelationship>>& relationships() { return parser_type_data_.relationships; }
  std::map<std::string, NonterminalID>& nonterminals_for_type() { return parser_type_data_.nonterminals_for_type; }
  ...
  const TypeDescription* deduceTypesDFS(FieldTracker& target_field,
