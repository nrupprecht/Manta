...
struct ProductionRule {
  ...
  void SetSourceNonterminal(NonterminalID nonterminal_id) { produced_nonterminal = nonterminal_id; }

  ProductionRule RebindSourceNonterminal(NonterminalID nonterminal_id) const {
    return ProductionRule(nonterminal_id, rhs);
  }
...
struct Instructions {
  // Assignment, e.g.
  // @.field_name = $0
  // @.field_name = $0.other_name
  // @.field_name = fnct(...args...)

  struct Assignment {
    std::string field_name;
  };

  //! \brief Record if the node name is specified.
...
struct AnnotatedProductionRule {
  ...
  std::shared_ptr<struct ParseNode> instructions = nullptr;
