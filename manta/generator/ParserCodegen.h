...
public:
  ...
  void GenerateParserCode(std::ostream& code_out, std::istream& parser_description, ParserType parser_type);
  ...
  //! \brief Get the parser generator, used to create the parser data.
  const ParserGenerator& GetParserGenerator() const { return generator_; }

  void SetParserGeneratorLogger(const lightning::Logger& logger) {
    generator_logger_ = logger;
    generator_.SetLogger(logger);
  }
...
private:
  ...
  ParserGenerator generator_ {ParserType::LALR};

  lightning::Logger generator_logger_;
};
}  // namespace manta