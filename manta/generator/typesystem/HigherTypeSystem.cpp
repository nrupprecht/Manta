...
ObjectPrototype::ObjectPrototype(std::string name, std::size_t num_type_vars)
    : ObjectPrototype(std::move(name), "", num_type_vars) {}

ObjectPrototype::ObjectPrototype(std::string name, std::string namespace_name, std::size_t num_type_vars)
...
std::string ObjectPrototype::Write() const {
  return "object " + namespace_name_ + "::" + write(object_name_, num_type_vars_);
}

std::string ObjectPrototype::Write(const SubstitutionCallback& substitution_callback,
                                   std::span<std::size_t> vars) const {
  std::ostringstream out;
  out << "object " << namespace_name_ << "::" << object_name_;
  if (0 < num_type_vars_) {
    out << "<";
    for (std::size_t i = 0; i < num_type_vars_; ++i) {
      substitution_callback(out, i, vars);
...
void ObjectPrototype::AddField(const std::string& field_name,
  ...
  fields_.emplace_back(
      Field {field_name, ElaboratedType {.type = field_type, .bindings = std::move(binding)}});
}
...
void ObjectPrototype::AddParent(const GeneralizedType* parent, std::vector<std::size_t> bindings) {
  MANTA_REQUIRE(parent->GetPrototypeType() == PrototypeType::OBJECT, "parent must be an object");
  MANTA_REQUIRE(parent->GetNumFreeVars() == bindings.size(),
                "all free variables in the parent must be bound");
  parent_types_.emplace_back(parent, std::move(bindings));
}
...
const std::vector<ObjectPrototype::Parent>& ObjectPrototype::GetParentTypes() const {
...
std::string EnumeratedPrototype::Write() const {
  return "enum " + namespace_name_ + "::" + write(enum_name_, num_type_vars_);
}

std::string EnumeratedPrototype::Write(const SubstitutionCallback& substitution_callback,
                                       std::span<std::size_t> vars) const {
  std::ostringstream out;
  out << "enum " << namespace_name_ << "::" << enum_name_;
  if (0 < num_type_vars_) {
    out << "<";
    for (std::size_t i = 0; i < num_type_vars_; ++i) {
      substitution_callback(out, i, vars);
...
ObjectPrototype* GeneralizedTypeSystem::RegisterObjectPrototype(const std::string& object_name,
  ...
  return RegisterObjectPrototype(object_name, "", num_type_vars);
}

ObjectPrototype* GeneralizedTypeSystem::RegisterObjectPrototype(const std::string& object_name,
                                                                const std::string& namespace_name,
                                                                std::size_t num_type_vars) {
  ObjectPrototype prototype(object_name, namespace_name, num_type_vars);
  auto hash = prototype.Hash();
  if (auto it = prototypes_.find(hash); it != prototypes_.end()) {
...
EnumeratedPrototype* GeneralizedTypeSystem::RegisterEnumeratedPrototype(
    ...
    std::vector<std::pair<std::string, const GeneralizedType*>> options,
    std::string namespace_name) {
  EnumeratedPrototype prototype(std::move(enum_name), std::move(options), std::move(namespace_name));
  auto hash = prototype.Hash();
  if (auto it = prototypes_.find(hash); it != prototypes_.end()) {
...
EnumeratedPrototype* GeneralizedTypeSystem::RegisterEnumeratedPrototype(std::string enum_name,
                                                                        ...
                                                                        std::string namespace_name) {
  std::vector<std::pair<std::string, const GeneralizedType*>> options_map;
  for (const auto& option : options) {
    options_map.emplace_back(option, type);
  }
  return RegisterEnumeratedPrototype(std::move(enum_name), std::move(options_map), std::move(namespace_name));
}
...
const GeneralizedType* GeneralizedTypeSystem::GetObjectType(const std::string& object_name,
                                                            std::size_t num_type_vars) {
  return GetObjectType(object_name, "", num_type_vars);
}

const GeneralizedType* GeneralizedTypeSystem::GetObjectType(const std::string& object_name,
                                                            const std::string& namespace_name,
                                                            std::size_t num_type_vars) {
  const auto proto = RegisterObjectPrototype(object_name, namespace_name, num_type_vars);
  return GetType(proto);
}

GeneralizedType* GeneralizedTypeSystem::GetEnumerationType(
    const std::string& enum_name,
    const std::vector<std::pair<std::string, const GeneralizedType*>>& options) {
