...
void TypeDescriptionStructure::AddField(const std::string& field_name, const TypeDescription* field_type) {
  LOG_SEV(Trace) << "Adding field " << type_name << "::" << field_name << " with type "
                 << field_type->Write();
  if (auto it = std::ranges::find_if(fields, [&field_name](auto& pr) { return pr.first == field_name; });
      it == fields.end())
  {
    // New field.
...
void TypeDescriptionStructure::AddConstructor(const StructureConstructor& constructor) {
  ...
  for (auto& [field, _] : constructor.additional_initializations) {
    MANTA_REQUIRE(ContainsField(field), "field '" << field << "' is not a field of " << Write());
  }
...
bool TypeDescriptionStructure::ContainsField(const std::string& name) const {
  return std::ranges::any_of(fields, [&name](const auto& field) { return field.first == name; });
}
