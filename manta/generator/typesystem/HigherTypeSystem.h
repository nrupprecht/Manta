...
public:
  EnumeratedPrototype(std::string name,
  ...
  const std::vector<std::pair<std::string, const GeneralizedType*>>& GetOptions() const { return options_; }

private:
  //! \brief The name of the enumeration.
  const std::string enum_name_;

  //! \brief Optionally a namespace for the type to live in.
  ...
  //! \brief The enumeration options - stored as a pair instead of a map to preserve the ordering.
  const std::vector<std::pair<std::string, const GeneralizedType*>> options_;
};
...
public:
  ...
  //! \brief Create an object with a namespace name.
  ObjectPrototype(std::string name, std::string namespace_name, std::size_t num_type_vars = 0);
  ...
  //! \brief Add a structure as a parent class of this structure. The type must be an object type.
  //!        TODO: Add bindings for the type variables in the parent.
  void AddParent(const GeneralizedType* parent, std::vector<std::size_t> bindings = {});
  ...
  const std::vector<Parent>& GetParentTypes() const;
...
private:
  //! \brief The name of the object. Objects are nominatively typed.
  ...
  std::vector<Parent> parent_types_;
...
public:
  ...
  ObjectPrototype* RegisterObjectPrototype(const std::string& object_name,
                                           const std::string& namespace_name,
                                           std::size_t num_type_vars = 0);

  EnumeratedPrototype* RegisterEnumeratedPrototype(
  ...
  const GeneralizedType* GetObjectType(const std::string& object_name,
                                       const std::string& namespace_name,
                                       std::size_t num_type_vars = 0);

  GeneralizedType* GetEnumerationType(
