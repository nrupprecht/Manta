...
namespace Manta {
  ...
  bool FiniteAutomatonNode::accept(const char c) const {
    for (auto &transition : transitions)
      if (transition.accept(c)) return true;
    return false;
  }
  ...
  void FiniteAutomatonNode::print() const {
    ...
    for (const auto tr : transitions) {
      cout << " -> " << tr.transition_state << ":";
      if (tr.lambda()) cout << "lambda\n";
      else cout << " [" << tr.range_initial << ", " << tr.range_final << "]\n";
    }
  ...
  Token FiniteAutomaton::get_token() {
    ...
    string literal;
    char c;
    auto get_char = [&] () { if (in->eof()) c=char(0); else in->get(c); }; // Lambda function returns char(0) for eof.
    auto put_char = [&] () { if (in->eof()); else in->putback(c); };
  ...
  //! \brief Return a FiniteAutomaton from this (assumedly) NFA.
  FiniteAutomaton FiniteAutomaton::nfa_to_dfa() {
    // States are sets of nfa states.
    vector<set<int> > dfa_states;
    dfa_states.push_back(set<int> {0}); // Starting state just contains 0.
    ...
    working_stack.push_back(make_pair(0, dfa_states[0]));
    dfa.add_node();
    ...
    while (!working_stack.empty()) {
      int state_id = working_stack.front().first;
      auto &state_set = working_stack.front().second;
  ...
  int FiniteAutomaton::add_node(FiniteAutomatonNode node) {
    dfa_nodes.push_back(node);
    return dfa_nodes.size()-1;
  }

  int FiniteAutomaton::add_node() {
    dfa_nodes.push_back(FiniteAutomatonNode());
    return dfa_nodes.size()-1;
  }
  ...
  inline bool FiniteAutomaton::will_accept(char c) {
    for (auto &transition : dfa_nodes[state_pointer].transitions)
      if (transition.accept(c)) {
        state_pointer = transition.transition_state;
        return true;
      }
    // No transition accepts this character.
    return false;
  }

  inline int FiniteAutomaton::accepting_state_lambda(const unsigned index) {
    // Check if the node itself is accepting.
    if (0<=dfa_nodes[index].accepting_state) return dfa_nodes[index].accepting_state;
    // Check for lambda transitions.
    std::deque<unsigned> lambda_stack;
    set<unsigned> lambda_set;
    ...
    while (!lambda_stack.empty()) {
      int node_id = lambda_stack.front();
  ...
  inline void FiniteAutomaton::compute_goto(
    ...
    auto contains = [&] (set<int>& state_set_1) {
      for (auto &state_set_0 : dfa_states) if (state_set_0==state_set_1) return true;
      return false;
    };

    // Map from state to vector of char range that will transition to that state.
    map<int, vector<pair<char, char> > > transition_ranges;
    set<int> lambda_set;
    ...
    vector<tuple<set<int>, char, char> > all_transition_sets;
    ...
    for (auto &object : all_transition_sets) {
      ...
      if (!contains(transition_set)) {
        ...
        for (auto node_id : transition_set) {
          ...
          if (accepting_state!=-1) {
            ...
            if (accepting!=-1 && accepting!=accepting_state) 
              cout << "ERROR. Overlapping accepting states: " << accepting << " and " << accepting_state << " for node " << new_state_id << endl;
        ...
        working_stack.push_back(make_pair(new_state_id, transition_set));
      }
      // This is not a new set, but there are new transitions to this set.
        ...
        unsigned id = 0;
        for (; id<dfa_states.size(); ++id) if (dfa_states[id]==transition_set) break;
        // Add the transition.
        dfa.dfa_nodes[state_id].transitions.push_back(
          TransitionType {id, range_initial, range_final}
        );
      }
    }
  }

  inline void FiniteAutomaton::compute_transitions(const int node_id, map<int, vector<pair<char, char> > >& transition_ranges, set<int>& lambda_set) {
    // Record states that are reachable by lambda transitions.
    deque<int> lambda_reachable;
    // Compute states reachable for non-lambda, and populate a deque of states reachable by lambda transitions.
    for (auto &transition : dfa_nodes[node_id].transitions) {
      int next_node = transition.transition_state;
      // Transition range.
      ...
      if (transition_ranges.count(next_node)==0) {
        ...
        else transition_ranges.insert(make_pair(next_node, vector<pair<char,char> > {{transition.range_initial, transition.range_final}}));
      }
      // If not a lambda transition, add/combine character ranges.
      else if (!transition.lambda()) {
        // There is at least one transition range to get to this node. Check if this is a different one, or if it can be combined with existing transitions.
        bool combined = false;
        ...
        if (!combined) transition_ranges[next_node].push_back(make_pair(transition_initial, transition_final));
      }
    }
  ...
  inline void FiniteAutomaton::create_transition_sets(
    ...
    set<tuple<char, bool, int> > order_structure;
    for (auto range_obj : transition_ranges) {
      int node_id = range_obj.first;
      auto &range_vector = range_obj.second;
      for (auto pr : range_vector) {
        order_structure.insert(make_tuple(pr.first, false, node_id));
        order_structure.insert(make_tuple(pr.second, true, node_id));
      }
    }
    ...
    for (auto p=order_structure.begin(); p!=order_structure.end(); ++p) {
      ...
      if (!ends) {
        ...
        if (!first_time && character-1!=last_char && !current_states.empty()) 
          all_transition_sets.push_back(make_tuple(current_states, last_char, character-1));
        else first_time = false;
        last_char = character;
        // Find all ranges that start at this character.
        current_states.insert(get<2>(*p));
        auto q = p;
        ++q;
        while (q!=order_structure.end() && get<0>(*q)==character && get<1>(*q)==ends) {
          ++p; // p and q now match.
          current_states.insert(get<2>(*p));
          ++q;
        }
        ...
        // Add set and characters.
        if (last_bool)
          all_transition_sets.push_back(make_tuple(current_states, last_char, character));
        else 
          all_transition_sets.push_back(make_tuple(current_states, last_char+1, character));
        last_char = character;

        // Find all ranges that end at this character.
        current_states.erase(get<2>(*p));
        auto q = p;
        ++q;
        while (q!=order_structure.end() && get<0>(*q)==character && get<1>(*q)==ends) {
          ++p; // p and q now match.
