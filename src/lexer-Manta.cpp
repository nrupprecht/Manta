...
namespace Manta {
    ...
    Token Lexer::getNext() {
        ...
        while (!instream.eof()) {
            ...
            // Words
                ...
                if (key!=reserved_words.end()) {
                    return Token(key->second, acc);
                }
                else {
                    return Token(built_in_token[3], acc);
                }
            }
            // Strings
            else if (c=='"') {
            ...
            // Operators.
                ...
                if (op!=reserved_operators.end()) {
                    return Token(op->second, acc);
                }
                else {
                    return Token(built_in_token[4], acc);
                }
            }
            // EOF
            else {
    ...
    int Lexer::getBuiltInType(int i) {
        ...
        if (-1 < i && i < built_in_types) {
            ...
            std::cout << "ERROR: Asking for built in type " << i << " to be assigned an id.\n";
            std::cout << "This function thinks there are only " << built_in_types << " built in types.\n";
            return -1;
        }
    }
    ...
    int Lexer::addKeyword(const string& word) {
        ...
        if (it == reserved_words.end()) {
            reserved_words.emplace(word, next_lexeme_id);
            inverse_map.emplace(next_lexeme_id, word);
            ++next_lexeme_id;
    ...
    bool Lexer::isReserved(const string& test) const {
        // Check maps.
        if (reserved_operators.find(test) != reserved_operators.end()) {
            return true;
        }
        if (reserved_words.find(test) != reserved_words.end()) {
            return true;
        }
        // Couldn't find it.
        return false;
